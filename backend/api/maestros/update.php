<?php
/**
 * API de Maestros - Actualizar existente
 * POST /api/maestros/update.php
 */
...
try {
    ...
    if (!$stmt->execute()) {
        ...
        logMessage("Error SQL al actualizar maestro: " . json_encode($errorInfo), 'ERROR');
        sendError('Error en la base de datos: ' . $errorInfo[2], HTTP_SERVER_ERROR);
    }
    
    logMessage("Maestro actualizado - ID: $idMaestro", 'INFO');
    ...
    logMessage("Error de base de datos al actualizar maestro: " . $e->getMessage(), 'ERROR');
    sendError('Error de base de datos: ' . $e->getMessage(), HTTP_SERVER_ERROR);
} catch (Exception $e) {
    error_log("Exception: " . $e->getMessage());
    logMessage("Error general al actualizar maestro: " . $e->getMessage(), 'ERROR');
